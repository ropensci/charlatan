...
UserAgentProvider <- R6::R6Class(
  ...
  public = list(
    ...
    chrome = function(version_from = 13, version_to = 63,
      ...
      platforms <- c(
        sprintf(
          tmplt, self$linux_platform_token(),
          saf,
        ...
        sprintf(
          tmplt, self$windows_platform_token(),
          saf,
          super$random_int(version_from, version_to),
          super$random_int(build_from, build_to),
          saf
    ...
    firefox = function() {
      ...
      platforms <- c(
        sprintf(
          tmplt_win, self$windows_platform_token(),
          sub("_", "-", MiscProvider$new()$locale()),
          super$random_int(0, 2),
          super$random_element(ver)
        ),
        sprintf(
          tmplt_lin, self$linux_platform_token(),
          super$random_int(5, 7),
          super$random_element(ver)
        ),
        sprintf(
          tmplt_mac, self$mac_platform_token(),
    ...
    safari = function() {
      saf <- sprintf(
        "%s.%s.%s", super$random_int(531, 535),
      ...
      platforms <- c(
        ...
        sprintf(
          tmplt_ipod, super$random_int(3, 4),
