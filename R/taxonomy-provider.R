tax_genera <- c(
  "Acanthophyllum", "Achillea", "Achyrocline", "Acmella", "Aconitum",
...
tax_epithets <- c(
  "abbeokutae", "ackermani", "adolphii", "adunca", "aegilicum",
...
TaxonomyProvider <- R6::R6Class(
  ...
  public = list(
    ...
    locale = NULL,
    #' @description Create a new `TaxonomyProvider` object
    #' @return A new `TaxonomyProvider` object
    initialize = function() {
      if (is.null(self$locale)) {
        raise_class("TaxonomyProvider")
      }
    },
    ...
    species = function(authority = FALSE, date = FALSE) {
      ...
      if (authority) {
        lname <- super$random_element(PersonProvider_en_US$new()$person$last_names)
        if (date) {
